...
# options
integratePrecision: float = (
    300.0  # the higher value, the higher precision (also slower)
)
...
def integrate(f, mi: float, ma: float) -> complex:
    sum = complex(0.0, 0.0)
    deltax = (ma - mi) / integratePrecision
    x = mi
    while x <= ma:
        sum += f(x) * deltax
        x += deltax
...
def setPrecision(value):
    global integratePrecision
    integratePrecision = value


# fourier tools
def transform(target, order: int) -> dict:
...
def untransform(complexParameters: dict, t: float) -> complex:  # t should be in [0, 1]
    sum = complex(0.0, 0.0)
